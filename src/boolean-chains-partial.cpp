...
void find_optimal_chain(Chain &chain, size_t &current_best_length,
  ...
  if (chain.expressions.size() <= 6) {
    max = 5;
  } else if (chain.expressions.size() <= 9) {
    max = 3;
  } else {
    max = 1;
  }
...
int main(int argc, char *argv[]) {
  vector<Function> targets = {
      Function(~0b1011011111100011), Function(~0b1111100111100100),
      Function(~0b1101111111110100), Function(~0b1011011011011110),
      Function(~0b1010001010111111), Function(~0b1000111111110011),
  ...
  for (int i = 1; i < argc; i++) {
    chain.add_target(targets[atoi(argv[i])]);
  }

  if (chain.targets.size() < 1) {
    cout << "usage: boolean-chains-partial <target-index> [<target-index> ...]"
         << endl;
    exit(-1);
  }
  vector<uint32_t> start_indices;
