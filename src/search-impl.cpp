// search_impl.cpp
//
// Included from full-search.cpp with these macros predefined:
//   SEARCH_FUNC
//   UNSEEN_GET(v), UNSEEN_CLEAR(v), UNSEEN_CLEAR_COND(v,c),
//   UNSEEN_RESTORE(v), UNSEEN_INIT()
//   TARGET_GET(v), TARGET_INIT()
//
// And these available from full-search.cpp:
//   All constants, globals, CAPTURE_STATS_CALL, PRINT_PROGRESS, etc.

#define ADD_EXPRESSION(value, chain_size)                                      \
  {                                                                            \
    const uint32_t _v = (value);                                               \
    expressions[_expr_size] = _v;                                              \
    _expr_size += UNSEEN_GET(_v);                                              \
    UNSEEN_CLEAR(_v);                                                          \
  }
...
#define ADD_EXPRESSION_TARGET(value, chain_size)                               \
  {                                                                            \
    const uint32_t _v = (value);                                               \
    const uint32_t _a = UNSEEN_GET(_v) & TARGET_GET(_v);                       \
    expressions[_expr_size] = _v;                                              \
    _expr_size += _a;                                                          \
    UNSEEN_CLEAR_COND(_v, _a);                                                 \
  }
...
#define FORWARD_DFS(CS, PREV_CS, NEXT_CS)                                      \
  ...
  CAPTURE_STATS_CALL(CS)                                                       \
                                                                               \
  for (uint32_t i##CS = i##PREV_CS + 1, _limit_##CS = expressions_size[CS];    \
       i##CS < _limit_##CS; ++i##CS) {                                         \
    chain[CS] = expressions[i##CS];                                            \
    not_chain[CS] = ~chain[CS];                                                \
    choices[CS] = i##CS;                                                       \
    const uint8_t is_target = TARGET_GET(chain[CS]);                           \
                                                                               \
    if (PLAN_MODE) {                                                           \
      ...
      if (__builtin_expect(NEXT_CS + num_unfulfilled_targets == MAX_LENGTH,    \
        ...
        next_##CS : if (__builtin_expect(tmp_chain_size < MAX_LENGTH, 1)) {    \
          ...
          for (; j < expressions_size[tmp_chain_size]; ++j) {                  \
            total_chains++;                                                    \
            if (__builtin_expect(TARGET_GET(expressions[j]), 0)) {             \
...
static void SEARCH_FUNC(int argc, char *argv[]) {
  uint8_t unseen_plain[SIZE] __attribute__((aligned(64)));
  ...
  uint32_t expressions_size[25] __attribute__((aligned(64))) = {0};
  uint32_t tmp_chain_size;
  uint32_t generated_chain_size;
  uint32_t tmp_num_unfulfilled_targets;

  auto print_chain = [&](const uint32_t *ch, uint32_t cs) {
    printf("chain (%d):\n", cs);
    for (uint32_t i = 0; i < cs; i++) {
      ...
      printf(" = %s", std::bitset<N>(ch[i]).to_string().c_str());
      if (TARGET_GET(ch[i])) {
        printf(" [target]");
      }
      printf("\n");
  ...
  while (chain_size < start_indices_size) {
    ...
    num_unfulfilled_targets -= TARGET_GET(chain[chain_size]);
    chain_size++;
  }
...
#undef ADD_EXPRESSION_TARGET
#undef GENERATE_NEW_EXPRESSIONS
#undef FORWARD_DFS
#undef BACKTRACK_DFS
#undef SEARCH_FUNC
#undef UNSEEN_GET
#undef UNSEEN_CLEAR
#undef UNSEEN_CLEAR_COND
#undef UNSEEN_RESTORE
#undef UNSEEN_INIT
#undef TARGET_GET
#undef TARGET_INIT
