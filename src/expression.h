...
public:
  ...
  bool operator==(const Expression<N> &other) const {
    return type == other.type && f1 == other.f1 && f2 == other.f2;
  }
};

namespace std {
template <std::size_t N> struct hash<Expression<N>> {
  size_t operator()(const Expression<N> &expr) const {
    size_t hash_value = 0;
    hash_value ^= std::hash<int>{}(static_cast<int>(expr.type));
    hash_value ^= std::hash<Function<N>>{}(expr.f1) << 1;
    hash_value ^= std::hash<Function<N>>{}(expr.f2) << 2;
    return hash_value;
  }
};
} // namespace std
