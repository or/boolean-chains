...
The idea is that the first few instructions are less likely to be optimal due to
the greedy approach, so we want to try more of them. But as the chain gets
longer the greedy algorithm does a good job of identifying the best next steps,
so we try fewer and fewer options. This makes the search space manageable.
...
$F = \{f_k \, | \, 1 \leq k \leq t\}$ is the set of target functions. We demand
$x_k \not\in F$ for all inputs, otherwise just remove it from $F$.
...
length $c$, which is the input to the algorithm, given all the arrays set up in
Algorithm S.

*A1.* [Initialize.] Set $N(c) \leftarrow N(c-1)$.

*A2.* [Loop on instructions.] Set $h \leftarrow I(C(c))$ and for $0 \leq j < c$,
...
$I(N(c)) \leftarrow f$, and $N(c) \leftarrow N(c) + 1$.

** Performance
My latest version of this program processes around 400-420 million chains per
second on my MacBook Air (M2, 2023). On AWS Batch with Fargate capacity on 1
vCPU it processes around 150-200 million chains per second; so about 38-51
...
all very slow compared with the latest version.
...
Number of unique optimal chains: 3
...
Number of unique optimal chains: 91
...
Number of unique optimal chains: 6
...
Number of unique optimal chains: 1 (!)
...
Number of unique optimal chains: 35
...
Total chains generated: 2,623,870,804,835,610

Number of unique optimal chains: 0 [so far]
...
Number of unique optimal chains: 1270

The minimal length is 13, one step shorter than the solution known in the book.

Here is the first one found, by sheer coincidence it finds all target functions
at the end.
...
Number of unique optimal chains of length 16 or fewer: 0
